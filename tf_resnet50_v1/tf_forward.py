...
#    config.mlu_options.convert_graph = True
    ...
    with inf_graph.as_default():
        with tf.compat.v1.gfile.GFile(FLAGS.input_pb, 'rb') as fb:
...
#            tf.import_graph_def(graph_def, name='')
        ...
        with tf.compat.v1.Session(config=config) as session:
            ...
            input = [tf.compat.v1.get_default_graph().get_tensor_by_name(node) for node in input_node.split(",")]
            output = [tf.compat.v1.get_default_graph().get_tensor_by_name(node) for node in output_node.split(",")]
